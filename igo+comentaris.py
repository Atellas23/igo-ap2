...
def build_complete_traffic_data(highways: highway_list,
                                congestions: congestion_list) \
        -> traffic_data_list:
    '''Utility function to convert construct a list of Traffic_data elements
...
def plot_highways(highways: highway_list,
                  ...
                  size: int = 800) -> None:
    '''Plots the received Highway list into a map, giving the user insight about
    what streets exactly are documented in the internet data they downloaded.
    The plot is not shown but directly saved in a file, by default called
    'highway_plot.png'.
    '''
...
def color_decide(state: int) -> str:
    ...
    congestion state. Here is the color legend based on the congestions state:
...
def _set_congestion(tdata: Traffic_data, graph: nx.MultiDiGraph,
    ...
    for i in range(1, len(nn)):
        ...
        destiny = nn[i]
        try:
            path = ox.shortest_path(graph, origin, destiny, weight='length')
            ...
            try:
                path = ox.shortest_path(graph, destiny, origin, weight='length')
                for i in range(1, len(path)):
                    a = path[i-1]
                    b = path[i]
                    graph.adj[a][b][0]['congestion'] = tdata.state
...
def build_igraph(graph: nx.MultiDiGraph, traffic_data: traffic_data_list,
                 _debug_nodes: bool = False) -> Optional[list]:
    ''' Utility function that adds the attributes congestion and itime to every
    edge in the OSM graph. itime is calculated dividing the edge length by the
    speed limit multiplied by a factor given by the edge congestion.
    '''
...
def plot_path(igraph: nx.MultiDiGraph, ipath: list,
              ...
              size: int = 800) -> None:
    ''' Plots the received shortest_path list into a map. The plot is not shown
    but directly saved in a file, by default called 'path_plot.png'.
    '''
    city_map = StaticMap(size, size)
    try:
